...
@EdmAction(namespace = "Permisos", name = "Agregar", isBound = true) 
@EdmReturnType(type = "com.cairone.odataexample.Permiso")
public class UsuarioPermisoAgregarAction implements Operation<PermisoEdm> {
	...
	public PermisoEdm doOperation(ODataRequestContext requestContext, DataSourceFactory dataSourceFactory) throws ODataException {
		...
		if(!isSuitable) {
			throw new ODataBadRequestException("LA OPERACION SOLO ES APLICABLE A LOS USUARIOS");
		}
		...
		if(usuarioPermisoEntity != null) {
			throw new ODataBadRequestException(String.format("EL USUARIO %s YA TIENE ASIGNADO EL PERMISO %s", usuarioEntity, permisoEntity));
		}

		try {
			usuarioPermisoEntity = usuarioService.asignarPermiso(usuarioEntity, permisoEntity);
			PermisoEdm permisoEdm = new PermisoEdm(usuarioPermisoEntity.getPermiso());
			return permisoEdm;
		} catch (Exception e) {
