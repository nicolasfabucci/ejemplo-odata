...
@EdmAction(namespace = "Sectores", name = "Quitar", isBound = true) 
@EdmReturnType(type = "com.cairone.odataexample.PersonaSector")
public class PersonaSectorQuitarAction implements Operation<PersonaSectorEdm> {
	...
	public PersonaSectorEdm doOperation(ODataRequestContext requestContext, DataSourceFactory dataSourceFactory) throws ODataException {
		...
		if(personaSectorEntity == null) {
			throw new ODataEntityNotFoundException(String.format("LA PERSONA %s NO EXISTE EN EL SECTOR %s", personaEntity, sectorEntity));
		}
		

		try {
			personaService.quitarDeSector(personaEntity, sectorEntity);
			return null;
		} catch (Exception e) {
